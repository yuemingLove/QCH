...
#import "EnrollActivityVC.h"
#import "ActivityCell.h"
#import "ActivityDetailVC.h"
#import "PActivityModel.h"

@interface EnrollActivityVC ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation EnrollActivityVC
...
-(void)headerFreshing{
    ...
    [HttpCenterAction GetMyApplyActivity:UserDefaultEntity.uuid page:PAGE pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"userGuid"] complete:^(id result, NSError *error) {
...
-(void)footerFreshing{
    ...
    if ([_funlist count] > 0 && [_funlist count] % PAGESIZE == 0) {
            ...
            [HttpCenterAction GetMyApplyActivity:UserDefaultEntity.uuid page:[_funlist count]/PAGESIZE+1 pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"userGuid"] complete:^(id result, NSError *error){
            NSDictionary *dict=result[0];
            if ([[dict objectForKey:@"state"] isEqualToString:@"true"]) {
