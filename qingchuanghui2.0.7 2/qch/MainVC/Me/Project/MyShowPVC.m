...
@interface MyShowPVC ()<UITableViewDataSource,UITableViewDelegate>
...
-(void)headerFreshing{
    ...
    [HttpProjectAction GetPlaceProject:UserDefaultEntity.uuid type:1 page:PAGE pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"userGuid"] complete:^(id result, NSError *error) {
...
-(void)footerFreshing{
    if ([_funlist count] > 0 && [_funlist count] % PAGESIZE == 0) {
        [HttpProjectAction GetPlaceProject:UserDefaultEntity.uuid type:1 page:[_funlist count]/PAGESIZE+1 pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"userGuid"] complete:^(id result, NSError *error) {
            NSDictionary *dict=result[0];
            if ([[dict objectForKey:@"state"] isEqualToString:@"true"]) {
