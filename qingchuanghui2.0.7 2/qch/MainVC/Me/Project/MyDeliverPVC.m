...
@interface MyDeliverPVC ()<UITableViewDataSource,UITableViewDelegate>
...
-(void)headerFreshing{
    ...
    [HttpProjectAction GetSendProject:UserDefaultEntity.uuid state:-1 page:PAGE pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"userGuid"] complete:^(id result, NSError *error) {
...
-(void)footerFreshing{
    if ([_funlist count] > 0 && [_funlist count] % PAGESIZE == 0) {
        ...
        [HttpProjectAction GetSendProject:UserDefaultEntity.uuid state:-1 page:[_funlist count]/PAGESIZE+1 pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"userGuid"] complete:^(id result, NSError *error) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *url=[NSString stringWithFormat:@"%@%@",SERIVE_USER,[dict objectForKey:@"t_User_Pic"]];
    [cell.comImageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"loading_1"]];
    
    cell.title.text=[dict objectForKey:@"t_User_RealName"];
    cell.contentLabel.text=[NSString stringWithFormat:@"%@  %@",[dict objectForKey:@"PositionName"],[dict objectForKey:@"t_User_Commpany"]];
    cell.detail.text=[dict objectForKey:@"t_User_Remark"];
