...
@interface MyNewInfoViewController ()<UITableViewDataSource,UITableViewDelegate,XHImageViewerDelegate, MyDetailFirstCellDelegate, MyDetailSecondCellDelegate, MyDetailThirdCellDelegate, MyAccountDetailsCellDelegate>{
    //身份区别
    NSInteger style;
...
-(void)createHeadView{
    ...
    _statusImageView  = [[UIImageView alloc] initWithFrame:CGRectMake(_nameLabel.left - 1, _nameLabel.bottom + 5*SCREEN_WSCALE, 60*SCREEN_WSCALE, 20*SCREEN_WSCALE)];
    [_headerView addSubview:_statusImageView];
    
    
    UIButton *updateInfo=[UIButton buttonWithType:UIButtonTypeCustom];
    updateInfo.frame=CGRectMake(imageV.right+100*PMBWIDTH, imageV.top, SCREEN_WIDTH-150*SCREEN_WSCALE, imageV.height);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (status==0) {
            ...
            return 70*SCREEN_WSCALE;
        }
    }else if(indexPath.section==1) {
        return 230*SCREEN_WSCALE;
    }
    return 0;
}
