...
-(void)headerFreshing{
    ...
    [HttpCollegeAction GetCourse:key userGuid:UserDefaultEntity.uuid page:PAGE pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"key"] complete:^(id result, NSError *error) {
...
-(void)footerFreshing{
    ...
    if ([_funlist count] > 0 && [_funlist count] % PAGESIZE == 0) {
        ...
        [HttpCollegeAction GetCourse:key userGuid:UserDefaultEntity.uuid page:[_funlist count]/PAGESIZE+1 pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"key"] complete:^(id result, NSError *error) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CourseListCell *cell = (CourseListCell*)[tableView dequeueReusableCellWithIdentifier:@"CourseListCell"];
    if (cell == nil) {
        NSArray* nibs = [[NSBundle mainBundle] loadNibNamed:@"CourseListCell" owner:self options:nil];
        for (id oneObject in nibs) {
            if ([oneObject isKindOfClass:[CourseListCell class]]) {
                cell = (CourseListCell *)oneObject;
            }
        }
    }
