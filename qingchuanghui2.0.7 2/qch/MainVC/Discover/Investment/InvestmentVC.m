...
-(void)headerFreshing{
    ...
    [HttpInvestPlaceAction GetInvestPlaceList:PAGE pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"page"] complete:^(id result, NSError *error) {
...
-(void)footerFreshing{
    ...
    if ([_funlist count] > 0 && [_funlist count] % PAGESIZE == 0) {
        [HttpInvestPlaceAction GetInvestPlaceList:[_funlist count]/PAGESIZE +1 pagesize:PAGESIZE Token:[MyAes aesSecretWith:@"page"] complete:^(id result, NSError *error) {
            NSDictionary *dict=result[0];
            if ([[dict objectForKey:@"state"]isEqualToString:@"true"]) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *path=[NSString stringWithFormat:@"%@%@",SERIVE_IMAGE,[dict objectForKey:@"t_InvestPlace_ConverPic"]];
    [cell.pImageView sd_setImageWithURL:[NSURL URLWithString:path] placeholderImage:[UIImage imageNamed:@"loading_1"]];
    cell.themeLabel.text = [dict objectForKey:@"t_InvestPlace_Title"];
    cell.nameLabel.text = [NSString stringWithFormat:@"入驻成员 %@人  投资案例 %@",[dict objectForKey:@"MemberCount"],[dict objectForKey:@"CaseCount"]];
    cell.contentLabel.text = [NSString stringWithFormat:@"投资金额: %@",[dict objectForKey:@"t_InvestPlace_Money"]];
    ...
    NSArray *invArray=(NSArray*)[dict objectForKey:@"InvestPhase"];
    if ([invArray count]>0) {
        NSDictionary *item=invArray[0];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dict = [_funlist objectAtIndex:indexPath.row];
    InvestDetail *invest = [[InvestDetail alloc]init];
    invest.Guid = [dict objectForKey:@"Guid"];
