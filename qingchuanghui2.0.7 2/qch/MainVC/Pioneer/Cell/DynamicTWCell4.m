...
-(void)updateData:(DynamicModel*)model{
    ...
    CGFloat height=60+size.height+10;
    ...
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(_contentLabel.left,height,_contentLabel.width*PMBWIDTH,25*SCREEN_WSCALE)];
    ...
    relayBtn.frame = CGRectMake(self.headImageView.left+15*PMBWIDTH,height,70*PMBWIDTH,25*SCREEN_WSCALE);
...
- (void)tapImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImageWithObject4:tap:)]) {
        [self.dyDelegate tapImageWithObject4:self tap:tap];
    }
}

- (void)tapheadImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImg4:tap:)]) {
        [self.dyDelegate tapImg4:self tap:tap];
    }
}

- (void)careClicked:(UIButton*)sender{
    
    if ([self.dyDelegate respondsToSelector:@selector(careClicked4:index:)]) {
        [self.dyDelegate careClicked4:self index:[sender tag]];
    }
}

- (void)share:(UIButton *)sender{
    if ([self.dyDelegate respondsToSelector:@selector(shareClicked4:index:)]) {
        [self.dyDelegate shareClicked4:self index:[sender tag]];
    }
}

- (void)TalkClicked:(UIButton *)sender{
    if ([self.dyDelegate respondsToSelector:@selector(talkClicked4:index:)]) {
        [self.dyDelegate talkClicked4:self index:[sender tag]];
    }
}
...
//    frame.size.height = size.height+52+23+index+65*PMBHEIGHT;
    ...
    frame.size.height=size.height+60+index+hegiht+20;
