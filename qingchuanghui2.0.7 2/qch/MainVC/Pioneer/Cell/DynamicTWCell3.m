...
- (void)tapheadImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImg3:tap:)]) {
        [self.dyDelegate tapImg3:self tap:tap];
    }
}
...
-(void)updateData:(DynamicModel*)model{
    ...
    if (self.contentLabel.height>98*PMBWIDTH) {
        height=60+size.height+10+14*PMBWIDTH;
    }else{
        height=60+size.height+10;
    }
    ...
    UILabel *linelab = [[UILabel alloc]initWithFrame:CGRectMake(25, self.frame.size.height-6, ScreenWidth-25, 1)];
...
- (void)tapImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImageWithObject3:tap:)]) {
        [self.dyDelegate tapImageWithObject3:self tap:tap];
    }
}

- (void)careClicked:(UIButton*)sender{
    
    if ([self.dyDelegate respondsToSelector:@selector(careClicked3:index:)]) {
        [self.dyDelegate careClicked3:self index:[sender tag]];
    }
}
- (void)share:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(shareClicked3:index:)]) {
        [self.dyDelegate shareClicked3:self index:[sender tag]];
    }
}
- (void)TalkClicked:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(talkClicked3:index:)]) {
        [self.dyDelegate talkClicked3:self index:[sender tag]];
    }
}
- (void)remove:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(deleteClicked3:index:)]) {
        [self.dyDelegate deleteClicked3:self index:[sender tag]];
    }
}
...
-(void)setIntroductionText:(NSString*)text index:(NSInteger)index{
    ...
    frame.size.height=size.height+60+index+hegiht+25;
