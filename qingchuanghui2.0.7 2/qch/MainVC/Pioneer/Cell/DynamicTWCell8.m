...
- (void)tapImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImageWithObject8:tap:)]) {
        [self.dyDelegate tapImageWithObject8:self tap:tap];
    }
}

- (void)tapheadImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImg8:tap:)]) {
        [self.dyDelegate tapImg8:self tap:tap];
    }
}

- (void)careClicked:(UIButton*)sender{
    
    if ([self.dyDelegate respondsToSelector:@selector(careClicked8:index:)]) {
        [self.dyDelegate careClicked8:self index:[sender tag]];
    }
}

- (void)share:(UIButton *)sender{
    if ([self.dyDelegate respondsToSelector:@selector(shareClicked8:index:)]) {
        [self.dyDelegate shareClicked8:self index:[sender tag]];
    }
}

- (void)TalkClicked:(UIButton *)sender{
    if ([self.dyDelegate respondsToSelector:@selector(talkClicked8:index:)]) {
        [self.dyDelegate talkClicked8:self index:[sender tag]];
    }
}
