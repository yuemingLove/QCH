...
//    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:bounds byRoundingCorners:UIRectCornerBottomLeft | UIRectCornerTopLeft cornerRadii:CGSizeMake(15, 15)];
...
- (void)tapImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImageWithObject:tap:)]) {
        [self.dyDelegate tapImageWithObject:self tap:tap];
    }
}

- (void)careClicked:(UIButton*)sender{
    
    if ([self.dyDelegate respondsToSelector:@selector(careClicked:index:)]) {
        [self.dyDelegate careClicked:self index:[sender tag]];
    }
}
- (void)share:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(shareClicked:index:)]) {
        [self.dyDelegate shareClicked:self index:[sender tag]];
    }
}
- (void)TalkClicked:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(talkClicked:index:)]) {
        [self.dyDelegate talkClicked:self index:[sender tag]];
    }
}
- (void)remove:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(deleteClicked:index:)]) {
        [self.dyDelegate deleteClicked:self index:[sender tag]];
    }
}
