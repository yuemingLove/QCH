...
- (void)tapheadImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImg7:tap:)]) {
        [self.dyDelegate tapImg7:self tap:tap];
    }
}
...
//    _collectBtn.frame = _chatbtn.frame;
    ...
    if ([model.ifPraise isEqualToString:@"1"]) {
        ...
        [_collectBtn setTitleColor:TSEColor(124, 197, 255) forState:UIControlStateNormal];
    }
...
- (void)tapImage:(UITapGestureRecognizer *)tap{
    if ([self.dyDelegate respondsToSelector:@selector(tapImageWithObject7:tap:)]) {
        [self.dyDelegate tapImageWithObject7:self tap:tap];
    }
}

- (void)careClicked:(UIButton*)sender{
    
    if ([self.dyDelegate respondsToSelector:@selector(careClicked7:index:)]) {
        [self.dyDelegate careClicked7:self index:[sender tag]];
    }
}
- (void)share:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(shareClicked7:index:)]) {
        [self.dyDelegate shareClicked7:self index:[sender tag]];
    }
}
- (void)TalkClicked:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(talkClicked7:index:)]) {
        [self.dyDelegate talkClicked7:self index:[sender tag]];
    }
}
- (void)remove:(UIButton *)sender
{
    if ([self.dyDelegate respondsToSelector:@selector(deleteClicked7:index:)]) {
        [self.dyDelegate deleteClicked7:self index:[sender tag]];
    }
}
