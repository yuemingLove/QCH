...
#import "RootPartnerClass.h"

@interface RootPartnerClass ()
@end
@implementation RootPartnerClass
...
-(instancetype)initWithDictionary:(NSDictionary *)dictionary
{
	...
	if(dictionary[@"result"] != nil && [dictionary[@"result"] isKindOfClass:[NSArray class]]){
		...
		for(NSDictionary * resultDictionary in resultDictionaries){
			PartnerResult * resultItem = [[PartnerResult alloc] initWithDictionary:resultDictionary];
...
-(NSDictionary *)toDictionary
{
	...
	if(self.result != nil){
		...
		for(PartnerResult * resultElement in self.result){
