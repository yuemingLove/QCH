...
-(void)commonInit {
    ...
    //按钮名称
    NSMutableArray *titleList = [[NSMutableArray alloc]initWithObjects:@"动态",@"合伙人", nil];
...
+ (instancetype)initWithTitleList:(NSMutableArray *)titleList {
    HomeSegment *segment = [[HomeSegment alloc]initWithTitleList:titleList];
...
-(void)moveToOffsetX:(CGFloat)offsetX {
    ...
    CGFloat addX = offSet/self.bounds.size.width*(bt2.center.x - bt1.center.x);
...
-(void)didSelectButton:(UIButton*)Button {
    ...
    switch (btn.tag) {
        case 1:
            ...
            break;
        case 2:
            [bt1 setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
