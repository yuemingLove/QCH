...
{
    return [self initWithMovieURL:newMovieURL size:newSize fileType:AVFileTypeQuickTimeMovie outputSettings:nil compressVideoOutput:NO];
}

- (id)initWithMovieURL:(NSURL *)url size:(CGSize)size compressVideoOutput:(BOOL)compress {
    return [self initWithMovieURL:url size:size fileType:AVFileTypeQuickTimeMovie outputSettings:nil compressVideoOutput:compress];
}



- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType outputSettings:(NSMutableDictionary *)outputSettings compressVideoOutput:(BOOL)compressVideo
{
    if (!(self = [super init]))
    {
		return nil;
    ...
    [self initializeMovieWithOutputSettings:outputSettings compressVideoOutput:compressVideo];
...
- (void)initializeMovieWithOutputSettings:(NSDictionary *)outputSettings compressVideoOutput:(BOOL)compressVideo {
    isRecording = NO;
    ...
    [compressionProperties setObject:[NSNumber numberWithInt: 1000000] forKey:AVVideoAverageBitRateKey]; //roughly 999kb
    [compressionProperties setObject:[NSNumber numberWithInt: 16] forKey:AVVideoMaxKeyFrameIntervalKey];
    [compressionProperties setObject:AVVideoProfileLevelH264Main31 forKey:AVVideoProfileLevelKey];
 
    if(compressVideo){
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:outputSettings];
